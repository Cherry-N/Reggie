...
public class CommonController {
    ...
    public R<String> upload(MultipartFile file) {
        ...
        String filename = UUID.randomUUID().toString() + suffix;   //cyjtdydt.jpg
        //创建一个目录对象
        File dir = new File(baseParh);
        //判断当前目录是否存在
        if (!dir.exists()) {
            //目录不存在 则创建
            dir.mkdir();
        ...
        //将临时文件转存到指定位置
        try {
            file.transferTo(new File(baseParh + filename));
        } catch (IOException e) {
    ...
    public void download(String name, HttpServletResponse httpServletResponse) {
        try {
            //输入流，通过输入流读取文件内容
            FileInputStream fileInputStream = new FileInputStream(new File(baseParh + name));
